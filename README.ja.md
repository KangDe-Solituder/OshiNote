...
- 既存の JSON、Markdown、TXT の読みやすいエクスポートは引き続き利用できます。
- 持ち運び可能な OshiNote ZIP バックアップを 2 種類作成できます：
  - データのみ：ローカルメディアを除くデータベース内容；
  - 完全データ：データベースに加えてローカル画像、サムネイル、スタンプフォント。
- データページからバックアップをインポートでき、データベース置換後にアプリを再起動します。
- 任意の WebDAV スナップショット同期で接続テスト、完全バックアップのアップロード、ダウンロードと復元を行えます。
- 初版の WebDAV はスナップショット方式で、双方向のレコード統合は行いません。
